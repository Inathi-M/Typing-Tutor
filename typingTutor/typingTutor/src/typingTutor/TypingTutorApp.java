...
//shared class variables
	...
	static SlidingWord horizontalWord = new SlidingWord();
	static HungryWordMover horizontalMover;
	...
	public static void setupGUI(int frameX,int frameY,int yLimit) {
		...
		gameWindow = new GamePanel(words,horizontalWord,yLimit,done,started,won);
	   ...
	   textEntry.addActionListener(new ActionListener() {
	      public void actionPerformed(ActionEvent evt) { 
	    	  ...
	    	  if (!pause.get()) {
				  ...
				  //Catch the lowest word
				  FallingWord lowerWord = null;
				  int index = -1;
				  for(int i = 0;i< noWords;i++){
                      if(text.equals(words[i].getWord())){
                           if(lowerWord == null || lowerWord.getY() < words[i].getY()){
                                          lowerWord = words[i];
										  index = i;
						   }
					  }
	    		  ...
	    		  CatchWord catchThread = new CatchWord(text,index);
		...
		quitB.addActionListener(new ActionListener() {
				  public void actionPerformed(ActionEvent e) {
					 ...
					 //word movers waiting on starting line
					   	for (int i=0;i<noWords;i++) {
					     		try {
									if (horizontalMover.isAlive()) {
										horizontalMover.join();
									}
...
public static void main(String[] args) {
	    ...
	    CatchWord.setSlidingWord(horizontalWord);
